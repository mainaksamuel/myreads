...
const Dashboard = ({ bookshelves }) => {
  ...
  const [bookShelfReads, setBookShelfReads] = useState({
    currentlyReading: [
      {
  ...
  const handleBookshelfUpdate = (updatedRead, newShelf, oldShelf) => {
    setBookShelfReads(() => {
      const updatedBooksInOldShelf = bookShelfReads[oldShelf]
        .filter(({ title }) => title !== updatedRead.title);

      if (newShelf === "none") return { ...bookShelfReads, [oldShelf]: [...updatedBooksInOldShelf] };

      const updatedBooksInNewShelf = bookShelfReads[newShelf].concat([updatedRead]);

      return { ...bookShelfReads, [oldShelf]: [...updatedBooksInOldShelf], [newShelf]: [...updatedBooksInNewShelf] }
    });
  }
